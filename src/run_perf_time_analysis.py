...
# metric can be temporarily excluded if it's very unstable and requires
...
def get_stats_data(url, days):
    ...
    search_results = [item for page_result in results for item in page_result]
...
def get_moving_averages(stats_data, window):
    for branch, branch_records in groupby(sorted(stats_data, key=lambda r: r.branch), lambda r: r.branch):
        for key, test_records in groupby(sorted(branch_records, key=lambda r: r.name), lambda r: r.name):
            ...
            moving_average = holt_winters_second_order_ewma(
                np.array(values), span=window, beta=0.3)
        ...
        class TestSequenceMeta(type):
            ...
            def __new__(mcs, name, bases, dict):
                ...
                def gen_test_long_raising_trend(moving_average):
                    def test(self):
                        deltas = zip(moving_average.value,
                                     moving_average.value[1:])
                        raising_trend = list(
                            reversed(list(takewhile(lambda (prv, nxt): nxt > prv, reversed(deltas)))))
                        if len(raising_trend) > 0:
                            long_threshold = 20
                            long_trend_percent = trend(
                                [min(moving_average.value[-window:]), raising_trend[-1][1]]) * 100
                ...
                for moving_average in moving_averages:
                    ...
                    property_name_long = 'test_long_' + \
                        moving_average.name.replace('.', '_')
                    dict[property_name_long] = gen_test_long_raising_trend(
                        moving_average)
